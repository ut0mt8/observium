...
function snmpwalkget_cache_oid($device, $oid, $array, $mib = NULL, $mibdir = NULL, $suboid, $flags = OBS_SNMP_ALL)
{
  if (is_bool($flags))
  {
    $flags = ($flags ? OBS_SNMP_ALL : OBS_SNMP_ALL ^ OBS_QUOTES_STRIP);
  ...
  $data = "";
  foreach ($suboid as $sub) {
    $res = snmp_get($device, "$oid.$sub", "-OQUs", $mib, $mibdir, $flags);
    $data .= "$res\n";
  }
