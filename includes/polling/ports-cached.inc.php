...
$data_oids = array_merge($data_oids_ifEntry,
                         $data_oids_ifXEntry,
                         array('ifTrunk', 'ifVlan')); // Add additional oids
...
$jsonfile = "/tmp/obs-poller-".$device['hostname'].".json";

/* cached logic */
if ($cached_json = json_decode(file_get_contents($jsonfile), true)) {
  $port_stats = $cached_json;
} else {
  // information cached
  ...
  $fp = fopen($jsonfile, 'w');
  fwrite($fp, json_encode($port_stats));
  fclose($fp);
}

$keys_oids = array_keys ($port_stats);

foreach ($stat_oids_ifEntry as $oid)
{
  echo("$oid ");
  $port_stats = snmpwalkget_cache_oid($device, $oid, $port_stats, "IF-MIB", mib_dirs(), $keys_oids);
}
